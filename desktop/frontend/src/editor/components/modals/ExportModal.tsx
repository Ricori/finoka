...
export function ExportModal() {
  ...
  async function go() {
    ...
    try {
      ...
      const r = await mediaLibrary.exportVideoRange(
        getVid(), baseName() + suffix + ".mp4", buildClipAss(T0, T1),
  ...
  return (
    <div className="modal" id="exp-modal" hidden={!open}
      ...
      <div className="box confirm exp">
        ...
        <div className="foot">
          <button className="btn" id="exp-cancel" disabled={busy} onClick={close}>
            {busy ? "正在导出…" : "取消"}
          </button>
